...
def compress_img_to_folder(path, prefig, quality, output):
    try:
        Folder = IMG(path=path, prefig=prefig,
                     quality=quality, output_path=output, extension='', count='')
...
def convert_img_to_folder(path, prefig, quality, output, extension, count):
    try:
        Folder = IMG(path=path, prefig=prefig,
                     quality=quality, output_path=output, extension=extension, count=count)
        Folder.get_files_of_path()
